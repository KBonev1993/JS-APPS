...
const detailsTemplate = (job, isOwner) => html`
        <!-- Details page -->
        <section id="details">
          <div id="details-wrapper">
            ...
            <div id="action-buttons">
              <a href="/details/${job._id}/edit" id="edit-btn">Edit</a>
              <a href="/details${job._id}/delete" id="delete-btn">Delete</a>

              <!--Bonus - Only for logged-in users ( not authors )-->
              <a href="/details${job._id}/apply" id="apply-btn">Apply</a>
            </div>
          </div>
        </section>
...
export const detailsView = (ctx) => {
  jobService.getOne(ctx.params.albumId).then((job) => {
    let isOwner = job._ownerId == ctx.user._id
    ctx.render(detailsTemplate(job, isOwner));
  });
};
